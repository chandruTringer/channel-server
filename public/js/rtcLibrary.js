...
Rtc.prototype.init = function(params) {
  ...
  try {
    ...
    tempObj.fullScreenToggleButton = (typeof params.fullScreenToggleButton === 'undefined') ? "fullScreenToggleButtonRtc" : params.fullScreenToggleButton;
    ...
    document.getElementById(tempObj.fullScreenToggleButton).onclick = (tempObj.fullScreenToggleHandler).bind(this);
...
Rtc.prototype.onChannelOpened = function() {
  ...
  var mediaAndChannelReady = (tempObj.mediaReady);
  var isAgent = (tempObj.room.user.agent);
  var isWaiting = (tempObj.room.user.waiting);
  // Is not agent and not waiting
  var isXAgentXWaiting = (!isAgent && !isWaiting); 

  tempObj.trace("Server", "Message", "Channel Opened for signalling.");

  if(mediaAndChannelReady && isXAgentXWaiting) {
    tempObj.sendMessage({
      type: "inRoom",
      userId: tempObj.room.user.userId,
      firstName: tempObj.room.user.firstName,
...
Rtc.prototype.fullScreenToggleHandler = function(){
  var tempObj = this;
